...
useEffect(() => {
      ...
      axios({
       ...
       st !== '' ?
          axios({
            url: appconfig.baseurl + "CommonUtility/City?stateId="+st,
            method: "GET",
            headers: { 'authorization': 'Bearer '+ BT  },
          }).then((res) => {
          ...
          <div className="registercontainer">
            ...
            { filedata === '' ? <span className="registerError registerErrorCenter"> {errors?.profilepic && errors.profilepic.message} </span> : null }
              ...
              <InputWrapperComponent
                ...
                <div className="registerField">
                  { }
                      {/* <select  name="state" className="registerSelect" {...register('state',  registerOptions.state)} > */}
                      <select  name="state" className="registerSelect" {...register('state',  registerOptions.state)}  onChange={ e => 
                        { setSt(e.target.value); setStname(e.target.options[e.target.selectedIndex].text); } }> 
                        <option value="" title="">Select State</option>
                         {
                            stateList.map((val) => <option value={val.id} title={val.name} key={val.id}>{val.name}</option>)
                         }
                      </select>
